#include "..\\..\\include\\游戏中机关\\SoftLand.h"
	...
	: times_passed(0)
{
	...
	sprite = ResourceManager :: getSpritePtr("SoftLand");//ÔÝÊ±ÕâÃ´Ð´£¬È¡µÃÌùÍ¼
	sprite -> SetZ(0.8f);
}
...
GameState SoftLand :: logic(Player* player, DoohSpecies* map)
{
	...
	float foot_y_center = player -> getY() + player -> getHeight() / 2;
	...
	if (species_center == SOFTLAND && 
		...
		&& player -> getX() >= x - 120 + player -> getWidth() / 2
		&& player -> getX() <= x + 120 - player -> getWidth() / 2
		&& player -> getY() >= y)
		times_passed = 3;
